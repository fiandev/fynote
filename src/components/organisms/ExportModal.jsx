...
export default function ExportModal ({ setExportModalOpened, children, items = [] }) {
    ...
    <Modal setExportModalOpened={setExportModalOpened}>
     ...
     <ModalContent>
      <form className="flex flex-col gap-4 p-2" onSubmit={handlerSubmit}>
        ...
        {
          showConfirmPassword && <InputFloating onInput={(e) => {
            setConfirmPassword(e.target.value);
            handler(e);
          }} label="confirm password" name="confirm-password" inValid={inValid ? "confirm password doesn't match !" : null} />
        }
